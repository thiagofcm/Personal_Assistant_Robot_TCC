        ...
        if objects:
            ...
            for obj in objects:
                ...
                if classes[obj.classid()] == target_object:
                    ...
                    obj_x_coordinate = int(((percent_location_x - in_min) * (out_max_motor - out_min_motor) / (in_max - in_min) + out_min_motor))
                    obj_y_coordinate = int(((percent_location_y - in_min) * (out_max_motor - out_min_motor) / (in_max - in_min) + out_min_motor))

                    #map function, or linear linearization of the inver_px value to the angle scale to get the angle of the servo:
                    object_angle_x = ((percent_location_x - in_min) * (out_max_motor - out_min_motor) / (in_max - in_min) + out_min_motor)*-1
                    object_angle_y = ((percent_location_x - in_min) * (out_max_motor - out_min_motor) / (in_max - in_min) + out_min_motor)*-1
                    ...
                    Servo(S2, int(object_angle_y))

                    uart_cord_message = '(' + str(obj_x_coordinate) + ',' + str(obj_y_coordinate) + ',' + str(linear_obj_area) + ')'
